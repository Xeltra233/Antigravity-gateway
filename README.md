...
- **查看版本信息**:
  ```bash
  ./gateway -v
  # 输出: Antigravity Gateway version 1.0.11 (commit: ..., built: ...)
  ```
...
- **存活探针 (Liveness)**: `GET /healthz` → 返回 `{"status":"ok","version":"1.0.11"}` (200 OK)
- **就绪探针 (Readiness)**: `GET /readyz` → 返回 `{"status":"ready","version":"1.0.11"}` (200 OK)
...
- **Liveness Probe**: `GET /healthz` → returns `{"status":"ok","version":"1.0.11"}` (200 OK)
- **Readiness Probe**: `GET /readyz` → returns `{"status":"ready","version":"1.0.11"}` (200 OK)
